...
action_update() {
    ...
    if [ "$OS" = "alpine" ]; then
        ...
        apk add --upgrade --repository=http://dl-cdn.alpinelinux.org/alpine/edge/community sing-box || {
            ...
            ( INSTALL_SCRIPT=$(mktemp /tmp/sb-install_sh.XXXXXX) && curl -fsSL https://sing-box.app/install.sh -o "$INSTALL_SCRIPT" && bash "$INSTALL_SCRIPT"; RET=$?; rm -f "$INSTALL_SCRIPT"; exit $RET ) || err "更新失败"
        }
    else
        ( INSTALL_SCRIPT=$(mktemp /tmp/sb-install_sh.XXXXXX) && curl -fsSL https://sing-box.app/install.sh -o "$INSTALL_SCRIPT" && bash "$INSTALL_SCRIPT"; RET=$?; rm -f "$INSTALL_SCRIPT"; exit $RET ) || err "更新失败"
    fi
